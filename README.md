...
For frontend: run "npm install" for react packages and "npm start" to start the application; Before running, install module "react-twitter-embed" and "react-chat-engine".
For backend: run "go build main" and "go run main.go" for the two backend under folder "backend" and "backend-Chat" seperatly. NOTE that we have two backend folders, need to run both folder seperatly with different terminals at the same time to make our web app work!
